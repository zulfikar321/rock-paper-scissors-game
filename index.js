...
function showResult(player, com, resultText) {
  ...
  const playgroundWrapper = document.querySelector(".playground__wrapper");
  playgroundWrapper.append(result);
  const playerPick = document.querySelector(".player");
  const comPick = document.querySelector(".com");
  showWinGradient(resultText, playerPick, comPick);
}

let result = "";
let scoreCount = document.querySelector(".score__count");
function scoreControl(result) {
  let score = "";
  if (result === "YOU WIN") {
    score += Number(scoreCount.innerText) + 1;
...
function playerChoise(choise) {
  const chooseOption = ["paper", "rock", "scissors"];
  const player = choise.id;
  const com = computerChoise(chooseOption);
  ...
  if (player === com) {
    ...
    result = com === "paper" ? "YOU LOSE" : "YOU WIN";
  } else if (player === "scissors") {
    result = com === "paper" ? "YOU WIN" : "YOU LOSE";
  }
  console.log(
    `you choose ${player} and computer choose ${com}, the result ${result}`
  );
  scoreControl(result);
  showResult(player, com, result);
}

const rulesModal = document.querySelector(".rules__modal");

function showRulesModal() {
  rulesModal.classList.add("show__modal");
}

function closeRulesModal() {
  rulesModal.classList.remove("show__modal");
}
